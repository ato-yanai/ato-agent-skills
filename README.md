...
> **前提:** このリポジトリは **private** です。取り込むには、GitHub に push 済みで、
> かつ取り込む人がその repo への**アクセス権を持ち、SSH 鍵を GitHub に登録済み**（`git clone git@github.com:...` が通る状態）である必要があります。

---

## 使う側：スキルをインストールする

private リポジトリなので、**SSH の git URL** で指定します。
`your-org` は実際の GitHub Organization／ユーザー名に置き換えてください。

利用できるスキルの一覧を見る:

```bash
npx skills add git@github.com:your-org/agent-skills.git --list
```
...
```bash
# 1つだけ
npx skills add git@github.com:your-org/agent-skills.git -s git-commit-message

# 複数指定 + インストール先エージェントを指定 + グローバル(~/.claude/skills 等)へ
npx skills add git@github.com:your-org/agent-skills.git -s git-commit-message -a claude-code -g

# 全部を非対話で取り込む
npx skills add git@github.com:your-org/agent-skills.git --all
```
...
`npx skills` の主要サブコマンド。`<source>` は SSH git URL（例 `git@github.com:your-org/agent-skills.git`）。
...
> このリポジトリは private なので **SSH git URL** を使う（前提は冒頭の「private」注記を参照）。
