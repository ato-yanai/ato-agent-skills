...
- **原則 `references/` に分割しない。説明・詳細・早見表は `SKILL.md` 本文に書く。**
  小さなスキルで安易に別ファイルへ散らすと段階読み込みの手間が増えるだけなので行わない。
  - **例外（大型・モード制スキルのみ）**：モードが複数あって本文が肥大化するスキルに限り、
    各モードの**手順だけ**を `modes/<mode>.md` に分割してよい。その場合も
    (1) 共通方針・共通規約・**起動時のモード選択は `SKILL.md` 本文に残す**、
    (2) `SKILL.md` から相対パスで参照する、
    (3) `references/` という名前のバケツ的分割（説明を意味なく散らす）はしない、を守る。
    例: [ato-design-doc](skills/ato-design-doc/SKILL.md)。
...
├── modes/            # 任意: 大型・モード制スキルのみ。モード別の手順（§0 の例外）
├── scripts/          # 任意: 実行可能なコード
...
説明・早見表・詳細は**原則すべて `SKILL.md` 本文に書く**。`references/` への分割は禁止
（§0。大型・モード制スキルの `modes/` 分割だけが例外）。
