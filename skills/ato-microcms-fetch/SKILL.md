...
> **前提:** 汎用 fetch ラッパー（`api-client.ts`）は [ato-fetch-client](../ato-fetch-client/SKILL.md) が提供する。
> 先にそれを適用して `api-client.ts` を配置し、本スキルのファイルを**同じディレクトリに**置く
> （`microcms.ts` は `./api-client` を import するため）。
...
1. **汎用クライアントを用意する。** [ato-fetch-client](../ato-fetch-client/SKILL.md) を適用し、
   `api-client.ts` をコピー先（既定 `src/lib/microcms/`）に置く。既にあれば再利用する。
