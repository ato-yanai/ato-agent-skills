...
仕組みは COMMON に1つ集約し、各FUNC/各画面はこれを参照する（DRYにするのは機構だけ）。
ドメイン固有の文言・軸はページ設計書(PAGE)/microCMS設計書(CMS)側に書く。
採番: 接頭辞 COMMON（=共通処理）、各仕様に AC。
-->
...
| COMMON | 名称 | 概要 | 利用する機能/画面 |
| --- | --- | --- | --- |
| COMMON-005 | 一覧フィルタ機構 | 絞り込み・ソートのクエリ規約と filter/orders 組み立て | FUNC-011/012/110, PAGE-003/PAGE-012 |
...
- 文言（画面タイトル・description）は各ページ設計書(PAGE)が指定し、組み立て規則のみ本機構が持つ。
